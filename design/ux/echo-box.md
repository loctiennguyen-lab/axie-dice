...
1. **Exact colors/spacing for the 6 new proposed CSS classes** (`.echohero`, `.echoshow`, `.echorank`, `.echopull`, `.echologrow`, `.fusepanel`) — the 6 classes shipped this session with real values (screenshot-confirmed, `production/session-state/active.md`); a separate full visual-QA pass on all 6 was not requested here and stays open if a future pass wants one. **RESOLVED (2026-09-07, art-director)** — the one specific piece this item explicitly deferred to this agent, the `.echoshow.locked` dimmed-unowned filter values, is decided below; nothing about locked-chip dimming remains open.

   **Decision — `.echoshow.locked` (dims the real thumbnail; never a "???" blackout, per §2).** The shipped placeholder (a programmer's guess) was `filter:grayscale(1) brightness(.9) contrast(.85);opacity:.65`. Two problems with it: (a) `grayscale(1)` strips 100% of the item's own color, which works against this exact panel's stated job — §2 says showing the real silhouette-but-dimmed art (not a flat "???") is what creates desire for the specific thing you don't own yet; a fully desaturated thumbnail throws away the color cues (a Mythic's gold accents, a specific avatar's palette) that let a player recognize and want *that* item. (b) `brightness(.9)` is only 10% darker than full strength — too subtle to read as "locked" sitting next to full-color owned chips in the same strip. Also directly on point: this session's Hero/Boss real-art pass (`production/session-state/active.md` item 8) found and fixed the mirror-image mistake elsewhere in the same codebase — a blanket CSS filter stack (`grayscale+sepia+hue-rotate`) was washing every piece of real monster/boss art into visual sameness. A heavy 3-filter stack here risks the identical failure mode on a different real-art surface.

   **Final values:**
   ```css
   .echoshow.locked .citem-thumb,.echoshow.locked .echoshow-swatch,.echoshow.locked .echoshow-pill{
     filter:grayscale(.65) brightness(.6);opacity:.55}
   .echoshow.locked .echoshow-n{opacity:.55}
   ```
   - `grayscale(.65)` — desaturates enough to read unambiguously as "muted/locked," while keeping ~35% of the item's own color so it still stays recognizable and desirable, not generic.
   - `brightness(.6)` — a decisive 40% darkening, unmistakable at a glance next to full-color owned chips in the same strip.
   - Dropped `contrast(.85)` — redundant with the brightness change; two clear levers (color, darkness) read more predictably than three subtle ones stacked together, and this is the same "don't stack filters you don't need" lesson from item 8 above.
   - `opacity:.55` on both the art and the name label (was `.65`/`.5`) — matches the existing `.citem.locked{opacity:.55}` value already used in Collection, so "locked" reads as the same dimming *amount* everywhere in the game even though the *method* differs by content type (Collection dims a flat "???" placeholder; Echo Box dims real art).
   - **Confirmed intentional, not changed**: `.echoshow.rN` border-color stays full-strength on locked chips (the CSS already scopes rarity-border rules outside `.locked`, so this needs no edit). Rarity is the entire "this is worth wanting" signal on a gacha banner — muting it alongside the art would blunt the one thing the banner exists to sell.
