...
| **CB-14b** | **Empty and hidden intent — Empty States Spec §1 (2026-09-21).** The enemy badge is **never hidden**: the column is a 186×370 stack with gap 8, so removing a 42px badge drags the status chips and HP bar up 44px mid-turn; a hole cannot tell "does nothing" from "not rolled yet"; and the badge is the intent tooltip's hover target. Three states, three shapes. **NO MOVE** — one element, the word, on `INERT_FILL` (#242A34, deliberately outside the die-type palette), ink `MUTED_TEXT`, Baloo 13 tracking .14em, padding 0/14, height 36, radius 10, 3px black, shelf 4, hugging to ~104 against a live badge's ~148. No icon tile, **no digit anywhere**, no divider, no target line — six separate signals so it can never be read as "0 damage". **HIDDEN** — the same plate but the 22px tile stays, holding the one `?` this screen may draw, beside the word `HIDDEN`. A party unit hides the Control entirely; that is the only hidden case. **Check** a blank enemy face renders NO MOVE with no digit glyph. **Visible** player. |
...
| **MAP-10** | **Current-node ring and label. SUPERSEDED IN PART by Empty States Spec §2 (2026-09-21) — see MAP-10b.** Ring inset −11, 5px `PRIMARY` with a 3px black halo, blinking 1.8s. Blink animates opacity, so the ring must be **shown/hidden by visibility, not by a static opacity** — an opacity-0 ring still lights up on every node when the animation drives it. Label chip below the token, height 30, radius 9, 3px black, shelf 3: current = `PRIMARY` / `INK_ON_PRIMARY` / `YOU ARE HERE`; open = `CREAM_RAISED` / `INK` / the node type name. Fogged, visited and skipped nodes have no chip. **Visible** player. |
| **MAP-10b** | **Ring inside, chip beside, pitch 124 — Empty States Spec §2 (2026-09-21).** MAP-10's geometry did not fit its own board: two tokens on a straight lane overlapped by 16px even with the chip deleted — 53 (half an open token) + 6 (shelf) + 61 (half the current token) + 14 (the ring's outside footprint) = 134 against a 118 pitch — and 52px with the chip in the vertical channel. Raising the pitch alone cannot fix it: keeping the chip under the token needs 170px a row, and seven of those do not fit under 1080. So all three move. **(a) Chip** anchors to the token's **right edge**, vertically centred, 10px clear of it (border included); height 30, radius 9, 3px black, shelf 3, padding 0/12, Baloo 14 tracking .08em; current = `PRIMARY` / `INK_ON_PRIMARY`, open = `CREAM_RAISED` / `INK`. It fits by measurement: lanes are 370 apart, widest token 122, leaving 248 clear; the widest chip (`YOU ARE HERE`) is ~152 and ends ~223 out. **(b) Ring** is drawn **inside** the token — inset +5, radius 19, 4px `PRIMARY`, **black halo deleted** (the token's own 5px outline already is one), z above the token body and below the icon disc, blink 1.8s opacity 1↔.35, zero footprint outside the token. **(c) Pitch** 118 → 124: r1 946 · r2 822 · r3 698 · r4 574 · r5 450 · r6 326 · r7 202; lanes 780 / 1150 / 1520 unchanged. **Check** the vertical channel is clear on every lane and r7 (token 94) does not reach the header at 96. **Visible** player. |
