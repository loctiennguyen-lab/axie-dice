# Axie roguelike icon set — 33 icons

26 of 33 are shipped Axie art, copied from source: 6 part icons from `dango-icons`, 20 from the Origins
asset kit (11 effects, all 6 nodes, and attack/heal/summon actions — summon uses `buff_eggshell.png`,
a cracked-egg motif). 6 had no asset and were
drawn to the kit's rules: energy, blank, buff, debuff, shard, reroll. Action's blank is a reused copy of
part/blank.svg.
...
| Slot | File | Note |
| --- | --- | --- |
| ATTACK | `action/attack.png` | kit `buff_dmg_boost.png`, buff arrow removed |
| HEAL | `action/heal.png` | kit `buff_healing_boost.png`, shipped as-is |
| BUFF | `action/buff.svg` | new — generic up-arrow, green |
| DEBUFF | `action/debuff.svg` | new — generic down-arrow, red |
| SUMMON | `action/summon.png` | kit `buff_eggshell.png`, shipped as-is |
| BLANK | `action/blank.svg` | reused — identical to `part/blank.svg` |
| REROLL | `action/reroll.svg` | new — circular arrow, blue |
| SHARD | `action/shard.svg` | new — faceted gem, cyan |

## Rules the new SVGs follow
...
- Buff: `#5FB026` / `#9FE85C`, sampled off the arrow in `buff_dmg_boost.png`. Debuff mirrors it in
  red, `#D6373D` / `#F08D91` — the kit has no generic debuff arrow to sample, so the red is chosen
  to match the danger tone already used across `debuff_*` icons.
- Shard: `#1F8FA6` / `#7FDCEC`, a cyan not used elsewhere in this set — no shard/gem asset exists in
  the kit, so this is drawn fresh as a faceted gem in the same construction as the others.
- Reroll: `#2E86D6` / `#7FC0F0`, blue — drawn as a thick circular arrow (ring + arrowhead flag) since the kit has no reroll/refresh glyph.
- Buff/debuff/shard's lighter block has no stroke of its own, so there's no seam line down the middle — only the outer silhouette is outlined.
