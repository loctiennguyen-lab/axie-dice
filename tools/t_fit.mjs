...
/* Dedicated boss-screen pass: teamPick/pickSeed above only ever lands on the
   FIRST node matching battle/elite/boss, which in 'short'/asc0/'fit' is always
   a plain battle node — 'boss' viewport-fit has never actually been exercised.
   Reach a real boss screen deterministically via devGo('boss') (same path
   devtools.js uses), on the narrowest viewport. */
{
  ...
  const r = await p.evaluate(()=>{
    ...
    return {zoom:document.querySelector('#app').style.zoom||'1',
      docH:document.documentElement.scrollHeight, winH:window.innerHeight,
      bottombarBottom: rb?Math.round(rb.bottom):null, trayBottom: rt?Math.round(rt.bottom):null,
      kind:S.kind, phase:S.phase, enemies:S.enemies.length};
  });
  const isBoss = r.kind==='boss';
  const ok = isBoss && r.bottombarBottom!=null && r.bottombarBottom <= r.winH+1;
  console.log(`${vp.width}x${vp.height}  BOSS SCREEN  kind=${r.kind} phase=${r.phase}  zoom=${r.zoom}  doc=${r.docH} win=${r.winH}  END TURN bottom=${r.bottombarBottom}  ${!isBoss?'NOT A BOSS NODE <-- TEST BROKEN':(ok?'VISIBLE ok':'OFF SCREEN <-- BUG')}`);
  await p.close();
}
