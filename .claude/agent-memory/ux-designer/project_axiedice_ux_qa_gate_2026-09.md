...
- **No accessibility toggle for full-screen flash/hitstop**: `flashScreen('gold'|'myth')` (full-viewport radial-gradient flash, `src/style.css` `.flashfx`) fires on every crit and Mythic pickup; `hitstop()` pulses a brightness/contrast filter on `#app`. `scSettings()` only exposes Volume, Animation Speed, Reference links, and Run actions — no way to reduce/disable flash or shake, which the studio's own accessibility checklist requires ("No flashing content without warning").
